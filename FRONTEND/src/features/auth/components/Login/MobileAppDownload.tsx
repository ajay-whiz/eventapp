...
const PlayStoreBadge: React.FC<{
  ...
  disabled?: boolean;
  surface?: 'dark' | 'light';
}> = ({ href, disabled, surface = 'dark' }) => {
  const isLight = surface === 'light';
  const content = (
    <>
      <svg
        className={`h-[18px] w-[18px] shrink-0 ${isLight ? 'text-slate-700' : 'text-slate-200'}`}
        viewBox="0 0 24 24"
        fill="currentColor"
        aria-hidden
      >
      ...
      <div className="flex flex-col">
        <span className={`text-[9px] uppercase tracking-wider ${isLight ? 'text-slate-500' : 'text-slate-500'}`}>
          Get it on
        </span>
  ...
  const className = `flex items-center gap-2 rounded-lg px-3.5 py-2 shrink-0 transition-opacity ${
    isLight ? 'bg-white/80 border border-slate-200' : 'bg-white/5 border border-white/10'
  }`;
...
const AppStoreBadge: React.FC<{ href: string; surface?: 'dark' | 'light' }> = ({
  href,
  surface = 'dark',
}) => {
  const isLight = surface === 'light';
  ...
  <a
    ...
    target="_blank"
    rel="noopener noreferrer"
    className={`flex items-center gap-2 rounded-lg px-3.5 py-2 shrink-0 hover:opacity-80 transition-opacity ${
      isLight ? 'bg-white/80 border border-slate-200' : 'bg-white/5 border border-white/10'
    }`}
  >
    <svg
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 shrink-0">
          <PlayStoreBadge
            ...
            surface={surface}
          />
          {appStoreUrl && <AppStoreBadge href={appStoreUrl} surface={surface} />}
        </div>
      </div>
    </div>
  );
