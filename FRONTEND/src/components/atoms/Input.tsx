...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(({
  ...
  const usesFixedHeight = /\bh-[\w\[\]-]+\b/.test(className);
  const sizeClasses = usesFixedHeight
    ? sizeMap[size].replace(/\bpy-\S+/g, 'py-0')
    : sizeMap[size];
      ...
      <div className="relative">
        ...
        {isPassword && (
          <button
            ...
            className="absolute inset-y-0 right-0 flex w-10 items-center justify-center text-gray-500 hover:text-gray-700 focus:outline-none cursor-pointer"
            tabIndex={-1}
